...
## load data
sw = read.csv("output/D47_d18sw_interpolation.csv")
...
png("figures/Fig3.regional_records.png", 6, 5.5, units = "in", res = 400)
# pdf("figures/Fig3.regional_records.pdf", width = 4, height = 6)
par(mar = c(4, 4, 1, 4))
plot(0, 0, xlim = c(2, 7.5), ylim = c(0, 5.2), axes = FALSE,
     xlab = "", ylab = "")
sw$d18sw.low = sw$d18sw - sw$d18sw.se
sw$d18sw.high = sw$d18sw + sw$d18sw.se
...
ob.am.rs = cbind(ob.am$age,
               4.2 + (ob.am$ob.am - min(tix)) / diff(range(tix)))
...
text(3, 5.2, "Lantian", col = pal[1], cex = 0.8)
text(3.7, 5.2, "Shilou", col =pal[2], cex = 0.8)
text(4.4, 5.2, "Jiaxian", col = pal[3], cex = 0.8)

text(2.2, 5, "a", font = 2)
text(7.2, 3.5, "b", font = 2)
text(7.2, 3, "c", font = 2)
text(7.2, 2, "d", font = 2)
text(7.2, 1.2, "e", font = 2)
text(2.2, 0.2, "f", font = 2)
